...
(function(e) {
	...
	var f = [ "01月", "02月", "03月", "04月", "05月", "06月", "07月", "08月", "09月",
			"10月", "11月", "12月" ], b = [ "日", "一", "二", "三", "四", "五", "六" ], i = /^(\+|\-)?(\d+)(M|Y)$/i, d = function(
			k, l) {
	...
	a.prototype = {
		...
		_initDateInput : function(l, o) {
			...
			k.className = "_initDateInput g_itext";
			k.id = l[0].id + "input";
			k.readOnly = true;
			k.type = "text";
			...
			e(k).on(
					"click",
					function() {
						var r = m._options, q = m._container || m.$el, p = e
								.proxy(m._eventHandler, m);
						m.minDate(r.minDate).maxDate(r.maxDate).date(
								r.date || new Date()).refresh();
						q.addClass("ui-calendar").on("click", p);
						r.swipeable && q.on("swipeLeft swipeRight", p);
						setTimeout(function() {
							m._showDate(q)
						}, 100)
					})
		...
		_eventHandler : function(s) {
			var q = this._options, l = (this._container || this.$el).get(0), p, r, k, o, m;
			switch (s.type) {
			case "swipeLeft":
			case "swipeRight":
				return this.switchMonthTo((s.type == "swipeRight" ? "-" : "+")
						+ "1M");
			case "change":
				m = e(
						".ui-calendar-header .ui-calendar-year, .ui-calendar-header .ui-calendar-month",
						this._container);
				return this.switchMonthTo(h(m.eq(1)), h(m.eq(0)));
			default:
				r = s.target;
				if ((p = e(r).closest(".ui-calendar-calendar tbody a", l))
						&& p.length) {
					s.preventDefault();
					k = p.parent();
					this
							._option("selectedDate", o = new Date(k
									.attr("data-year"), k.attr("data-month"), p
									.text()));
					...
					this._hideDate(this._container)
				} else {
					if ((p = e(r).closest(
							".ui-calendar-prev, .ui-calendar-next", l))
							&& p.length) {
						s.preventDefault();
						...
						if (((p = e(r).closest(".ui-calendar-return", l)) || (p = e(
								r).closest(".ui-calendar-cancel", l)))
								&& p.length) {
							s.preventDefault();
							this._hideDate(this._container)
						} else {
							if ((p = e(r).closest(".ui-calendar-return", l))
									&& p.length) {
								s.preventDefault();
								this._hideDate(this._container)
							} else {
								if ((p = e(r).closest(".ui-calendar-day", l))
										&& p.length) {
								}
							}
						}
		...
		_option : function(l, q) {
			var m = this._options, k, o, p;
			if (q !== undefined) {
				switch (l) {
				case "minDate":
				case "maxDate":
					m[l] = q ? e.calendar.parseDate(q) : null;
					break;
				case "selectedDate":
					o = m.minDate;
					p = m.maxDate;
					q = e.calendar.parseDate(q);
					q = o && o > q ? o : p && p < q ? p : q;
					m._selectedYear = m._drawYear = q.getFullYear();
					m._selectedMonth = m._drawMonth = q.getMonth();
					m._selectedDay = q.getDate();
					break;
				case "date":
					this._option("selectedDate", q);
		...
		refresh : function() {
			var m = this._options, l = this._container || this.$el, k = e
					.proxy(this._eventHandler, this);
			if (!m._invalid) {
				return
			}
			e(".ui-calendar-header select", l).off("change", k);
			l.empty().append(this._generateHTML());
			l.appendTo(e("#webFooterBox"));
			e(".ui-calendar-header select", l).on("change", k);
			m._invalid = false;
			return this
		},
		destroy : function() {
			var l = this._container || this.$el, k = this._eventHandler;
			e(".ui-calendar-header select", l).off("change", k);
			l.remove();
		...
		_renderHead : function(q, m, r, o, k) {
			var s = '<div class="ui-calendar-header">', l = new Date(m, r, -1), t = new Date(
					m, r + 1, 1), p, u;
			s += '<a class="ui-calendar-return"></a><div class="ui-calendar-title">';
			if (q.yearChangeable) {
				s += '<select class="ui-calendar-year">';
				for (p = Math.max(1930, m - 80), u = p + 100; p < u; p++) {
					s += '<option value="' + p + '" '
			...
			s += '</div><div class="ui-calendar-preandnext"><a class="ui-calendar-prev'
					...
					+ (k && k < t ? " ui-state-disable" : "")
					+ '" href="#"></a></div></div>';
			return s
		},
		_renderDay : function(r, o, l, s, t, u, q, k) {
			var m = (o.getMonth() !== s), p;
			p = m || (q && o < q) || (k && o > k);
			return "<td class='"
