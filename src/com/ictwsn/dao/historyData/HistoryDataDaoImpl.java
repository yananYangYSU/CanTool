...
public class HistoryDataDaoImpl extends MySQLBaseDao implements HistoryDataDao {
	...
	/**
	 * 实时数据显示dao层 的实现 将can信息和解析出的物理值通过一定的显示规则封装至map数组
	 * 
	 * @throws SQLException
	 */
	@Override
		...
		try {
			...
			while (rs.next()) {
				...
				if (rs.getString(2).length() == 3) {
					messageStr = "t" + rs.getString(2) + rs.getInt(3)
				...
				CanMsgDataBean canMsg = UncodeCanMsg.getInstance()
						.splitDataStr(messageStr);
				signal.add(messageStr);
				String idStr = null;
				String messageNameStr = null;
				if (id < 100) {
					idStr = id + "&nbsp;&nbsp;";
				} else {
					idStr = id + "";
				}
				if (messageName.length() == 5) {
					messageNameStr = messageName + "&nbsp;&nbsp;";
				} else {
					messageNameStr = messageName + "";
				}
				message = rs.getString(5).substring(0, 19)
						+ "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
						+ idStr
						+ "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
						+ messageNameStr
						...
						+ rs.getInt(3)
						+ "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
						+ rs.getString(4); // key由time,id.name,dcl,data组成
				canPhy = UncodeCanMsg.getInstance().parseCanData(
						UncodeCanMsg.getInstance().splitDataStr(messageStr));
				signal.add(message);
				for (int i = 0; i < canPhy.size(); i++) {
					CanPhyDataBean canphy = canPhy.get(i);
					String signalStr = "<tr><td width=\"140px\">"
							+ canphy.getSignalName()
							+ "</td><td width=\"140px\">"
							+ canphy.getPhyValue() + canphy.getUnit()
							+ "</td><td width=\"140px\">" + "["
							+ canphy.getHexStr().toString().toUpperCase() + "]"
							+ "</td><td width=\"140px\">" + "["
							+ canphy.getBinaryStr() + "]" + "</td></tr>";
	...
	@Override
	public Map<Integer, ArrayList<String>> QueryByTime(String startTime,
			String endTime, int number, int size) {
	...
	public Map<Integer, ArrayList<String>> SearchHistoryData(int number,
			int size) {
		Map<Integer, ArrayList<String>> map = new HashMap<Integer, ArrayList<String>>();
		try {
			conn = CurrentConn.getInstance().getConn();
