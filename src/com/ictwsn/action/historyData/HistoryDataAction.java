...
public class HistoryDataAction {
	private static Logger logger = Logger.getLogger(HistoryDataAction.class
			.getName());
	@Resource
	HistoryDataService hService;
	...
	public String showDataFabric(HttpServletRequest request,
		...
		try {
			...
			if (page > 1) {
				model.addAttribute("prePageHref", "showDataFabric.do?page="
						+ (page - 1));
			}
			if (page < maxPage) {
				model.addAttribute("nextPageHref", "showDataFabric.do?page="
						+ (page + 1));
			}
