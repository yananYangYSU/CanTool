...
public class HistoryDataAction {
	...
	@Resource
	ExportDataService oService;
	@Resource
	SystemSetService aService;
	...
	public String addClientBefore(HttpServletRequest request,
			HttpSession session, Model model) {
		try {
			RoleBean rb = (RoleBean) session.getAttribute("RoleBean");
			if (rb != null) {
				List<OperatorBean> operatorList = new ArrayList<OperatorBean>();
				if (rb.getRoleName() != null
						&& rb.getRoleName().equals("operator")) {
					OperatorBean ob = new OperatorBean();
					ob.setName(rb.getUserName());
					ob.setId(rb.getUserId());
					operatorList.add(ob);
				} else {
					operatorList = aService.searchOperator(rb.getUserId(), 0,
							9999, rb.getRoleName());// 查询出该用户下属的所有终端用户姓名及id
				}

				model.addAttribute("operatorList", operatorList);
				return GetHttpType.isMobileDevice(request) ? "pages/operator/addClient"
						: "MobilePages/operator/addClient";
			} else {
	...
	public String updateClientBefore(HttpServletRequest request,
			HttpSession session, Model model,
			@RequestParam(value = "clientId", required = true) int clientId) {
		try {
			...
			return GetHttpType.isMobileDevice(request) ? "pages/operator/updateClient"
					: "MobilePages/operator/updateClient";
		} catch (Exception e) {
	...
	public String searchClient(HttpServletRequest request, Model model,
		...
		try {
			...
			int size = 10; // 每页显示大小
			int maxPage = (totalCount % size == 0) ? totalCount / size
					: totalCount / size + 1;// 最大页数
			page = (page == 0) ? 1 : page; // 当前第几页
			int number = (page - 1) * size;

			List<ClientBean> cblist = oService.searchClient(userId, number,
					size, roleName); // 查询该用户对应数量的设备信息
			...
			if (page > 1) {
				model.addAttribute("prePageHref", "searchClient.do?userId="
						+ userId + "&page=" + (page - 1) + "&roleName="
						+ roleName);
			}
			if (page < maxPage) {
				model.addAttribute("nextPageHref", "searchClient.do?userId="
						+ userId + "&page=" + (page + 1) + "&roleName="
						+ roleName);
			}
			return GetHttpType.isMobileDevice(request) ? "pages/operator/searchClient"
					: "MobilePages/operator/searchClient";
		} catch (Exception e) {
			logger.error("login error" + e);
			e.printStackTrace();
			return "pages/error";
	...
	public String searchClientByCondition(HttpServletRequest request,
			Model model,
			@RequestParam(value = "page", required = true) int page,
			@RequestParam(value = "userId", required = true) int userId,
			@RequestParam(value = "roleName", required = true) String roleName,
		...
		try {
			int totalCount = oService.getClientCountByCondition(type, keyword,
					userId, roleName); // 查询该用户拥有的设备总数
			int size = 10; // 每页显示大小
			int maxPage = (totalCount % size == 0) ? totalCount / size
					: totalCount / size + 1;// 最大页数
			page = (page == 0) ? 1 : page; // 当前第几页
			int number = (page - 1) * size; // number 和size没有加入到sql的limit中
			List<ClientBean> cblist = oService.searchClientByCondition(type,
					keyword, userId, roleName, number, size);
			...
			model.addAttribute("totalCount", totalCount);
			if (page > 1) {
				model.addAttribute("prePageHref",
						"searchClientByCondition.do?userId=" + userId
								+ "&page=" + (page - 1) + "&roleName="
								+ roleName + "&keyword=" + keyword + "&type="
								+ type);
			}
			if (page < maxPage) {
				model.addAttribute("nextPageHref",
						"searchClientByCondition.do?userId=" + userId
								+ "&page=" + (page + 1) + "&roleName="
								+ roleName + "&keyword=" + keyword + "&type="
								+ type);
	...
	public String viewClient(HttpServletRequest request, Model model,
		...
		try {
			...
			return GetHttpType.isMobileDevice(request) ? "pages/operator/viewClient"
					: "MobilePages/operator/viewClient";
		} catch (Exception e) {
			logger.error("searchClientById error" + e);
			e.printStackTrace();
			return "pages/error";
